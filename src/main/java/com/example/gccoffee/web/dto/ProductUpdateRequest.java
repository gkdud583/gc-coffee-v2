...
public class ProductUpdateRequest {
    ...
    @NotNull(message = "상품 카테고리는 필수 입력입니다.")
    private final Category category;
    @NotBlank(message = "상품 이름은 필수 입력입니다.")
    private final String productName;
    @Min(value = 1, message = "가격은 최소 1원이어야 합니다.")
    private final int price;
    @Min(value = 0, message = "상품 수량은 최소 0개이어야 합니다.")
    private final int stockQuantity;
    @NotBlank(message = "상품 설명은 필수 입력입니다.")
    private final String description;

    public ProductUpdateRequest(Long productId, Category category, String productName, int price, int stockQuantity, String description) {
